...
def ensure_deps():
    ...
    # Add a safe, modern transformers stack for Colab (fixes GenerationMixin import issues)
    try:
...
def extract_features_for_candidate(pl: Dict, polar: Dict) -> np.ndarray:
    ...
    def pick(arr, frac):
        ...
        # ^ simple "quantile-like" picks (mid, q1, q3)
...
def _wing_coeffs_from_polar(pl: Dict, polar: Dict, aoa_root_deg: float, e0: float) -> Tuple[float,float,float]:
    ...
    mets = planform_metrics(y, c, pl["s"])
    S_full = mets["area_m2"]
    AR     = mets["aspect_ratio"]
...
def find_best_wing(airfoil_file, polar_file, objective,
    ...
    try:
        ...
        # ---- LLM explanation using a private top-k slice (k=5) ----
        k_for_llm = int(min(5, len(order)))
        sel = order[:k_for_llm]
        rows = []
        for r_idx, idx in enumerate(sel, start=1):
            _pl = plans[idx]
            _m  = planform_metrics(_pl["y"], _pl["cvec"], _pl["s"])
            rows.append(dict(
                rank=r_idx,
                score=float(probs[idx]),
                span_m=float(2.0*_pl["s"]),
                c_root_m=float(_pl["c_root"]),
                c_tip_m=float(_pl["c_tip"]),
                taper=float(_pl["taper"]),
                area_m2=float(_m["area_m2"]),
                aspect_ratio=float(_m["aspect_ratio"]),
                mac_m=float(_m["mac_m"]),
                washout_deg=float(_pl["twist"][-1] - _pl["twist"][0]),
            ))
        _df_llm = pd.DataFrame(rows)
        _best_row = _df_llm.iloc[0].to_dict()
        _rivals   = _df_llm.iloc[1:].reset_index(drop=True) if len(_df_llm) > 1 else pd.DataFrame()
        llm_text = _llm_compare_and_explain(objective, _best_row, _rivals)

        explanation = f"Objective: {objective}\n\n{llm_text}"
